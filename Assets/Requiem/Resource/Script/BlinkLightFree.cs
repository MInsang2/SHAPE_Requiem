...
public class BlinkLightFree : MonoBehaviour
{
    ...
    void LuminusBlink()
    {
        if (m_light.lightType == Light2D.LightType.Point)
        {
        ...
        else if (m_light.lightType == Light2D.LightType.Freeform)
        {
            if (m_light.shapeLightFalloffSize < m_maxRadius && !m_isMax)
            {
                DOTween.To(() => m_light.shapeLightFalloffSize, x => m_light.shapeLightFalloffSize = x, m_maxRadius, m_lightTime);
                m_delayTime += Time.deltaTime;
                if (m_delayTime >= m_lightTime)
                {
                    m_delayTime = 0f;
            ...
            if (m_light.shapeLightFalloffSize >= m_minRadius && m_isMax)
            {
                DOTween.To(() => m_light.shapeLightFalloffSize, x => m_light.shapeLightFalloffSize = x, m_minRadius, m_lightTime);
                m_delayTime += Time.deltaTime;
                if (m_delayTime >= m_lightTime)
                {
                    m_delayTime = 0f;
